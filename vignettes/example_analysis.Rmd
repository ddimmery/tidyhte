...
data %>%
    attach_config(cfg) %>%
    make_splits({{ userid }}, .num_splits = 6) %>%
...
construct_pseudo_outcomes(prepped_data, {{ outcome_variable }}, {{ treatment_variable }}, type = "dr") %>%
    estimate_QoI(!!!moderators) -> results_dr
```

```{r ipw}
construct_pseudo_outcomes(prepped_data, {{ outcome_variable }}, {{ treatment_variable }}, type = "ipw") %>%
    estimate_QoI(!!!moderators) -> results_ipw
```

```{r plugin}
construct_pseudo_outcomes(prepped_data, {{ outcome_variable }}, {{ treatment_variable }}, type = "plugin") %>%
    estimate_QoI(!!!moderators) -> results_plugin
```

```{r show_prep}
result_list <- list(dr=results_dr, ipw=results_ipw, plugin=results_plugin)
...
for (nm in names(result_list)) {
    print(nm)
    filter(result_list[[nm]], grepl("SATE|PATE", estimand))
}
```
...
```{r sl_coef}
plot_list <- list()
filter(results_dr, grepl("SL coefficient", estimand)) %>%
...
```{r sl_risk}
filter(results_dr, grepl("SL risk", estimand)) %>%
mutate(level = factor(level, levels = c("Control Response", "Treatment Response", "Effect Surface"))) %>%
ggplot() +
    geom_abline(intercept = 0, slope = 0, linetype = "dashed") +
    geom_pointrange(
...
```{r vimp}
for (nm in names(result_list)) {
ggplot(filter(result_list[[nm]], estimand == "VIMP")) +
    geom_abline(intercept = 0, slope = 0, linetype = "dashed") +
    geom_pointrange(
    ...
    coord_flip() +
    ggtitle("Covariate Importance", subtitle = nm) +
    theme_minimal() -> gp
plot_list <- c(plot_list, list(gp))
print(gp)
}
...
for (nm in names(result_list)) {
for (cov in continuous_moderators) {
    ggplot(filter(result_list[[nm]], estimand == "MCATE", term == cov)) +
        geom_abline(intercept = 0, slope = 0, linetype = "dashed") +
        geom_ribbon(
        ...
        ggtitle(paste("Marginal effects across", cov), subtitle = nm) +
        theme_minimal() -> gp
    plot_list <- c(plot_list, list(gp))
    print(gp)
}}
...
for (nm in names(result_list)) {
for (cov in continuous_moderators) {
    ggplot(filter(result_list[[nm]], estimand == "PCATE", term == cov)) +
        geom_abline(intercept = 0, slope = 0, linetype = "dashed") +
        geom_ribbon(
            aes(x = value, ymin = estimate - 1.96 * std_error, ymax = estimate + 1.96 * std_error), alpha = 0.75
        ) +
        ...
        scale_x_continuous("Covariate level") +
        scale_y_continuous("CATE") +
        ggtitle(paste("Partial effects across", cov), subtitle = nm) +
        theme_minimal() -> gp
    plot_list <- c(plot_list, list(gp))
    print(gp)
}}
