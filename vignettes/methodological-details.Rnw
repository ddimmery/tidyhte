...
library(dplyr)
devtools::load_all("~/GitHub/tidyhte")
#library(tidyhte)
library(magrittr)
...
\begin{figure}
<<discretemcate,echo=FALSE,results='hide',fig=FALSE>>=
filter(results, estimand == "MCATE", is.na(value)) %>%
ggplot(aes(level, estimate)) +
geom_point() +
geom_linerange(aes(ymin = estimate - 1.96 * std_error, ymax = estimate + 1.96 * std_error)) +
...
Similarly, it's easy to plot diagnostic information, as in Figures~\ref{fig:diag} and \ref{fig:coef}.
