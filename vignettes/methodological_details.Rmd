...
required_pkgs <- c("SuperLearner", "vimp", "quadprog", "glmnet", "nprobust", "ggplot2", "palmerpenguins")
...
```{r setup, echo=FALSE}
# This vignette uses the optional SuperLearner, vimp, glmnet, nprobust, ggplot2 and
# palmerpenguins packages; see the `check_suggests` chunk above.
suppressMessages({
...
```{r attach}
penguins <- penguins %>% attach_config(cfg)
```
...
```{r split}
penguins <- penguins %>% make_splits(id, species, sex, flipper_length_mm, .num_splits = 3)
```
...
penguins <- penguins %>% produce_plugin_estimates(
  # outcome
  food_consumed_g,
  # treatment
  treatment,
...
penguins <- penguins %>% construct_pseudo_outcomes(food_consumed_g, treatment)
```
...
results_mass <- penguins %>% produce_plugin_estimates(
  # outcome
  body_mass_g,
  # treatment
  treatment,
