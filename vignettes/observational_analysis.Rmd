...
output:
    rmarkdown::html_vignette:
        toc: true
vignette: >
  %\VignetteIndexEntry{HTE Analysis on Observational Data}
...
In this example analysis, I will demonstrate how to run an analysis of heterogeneous treatment effects in an observational setting using the methods of Kennedy (2023). 
...
This vignette uses the optional `SuperLearner` package (for the machine learning ensembles), `vimp` (for variable importance), `glmnet` and `nprobust` (for the specific learners and kernel smoothers used below) and `ggplot2` (for plotting). Install them with `install.packages(c("SuperLearner", "vimp", "glmnet", "nprobust", "ggplot2"))` if you have not already.

```{r rmd_setup, include = FALSE}
# Detect if we're on CRAN
on_cran <- !identical(Sys.getenv("NOT_CRAN"), "true")

# Set different figure options for CRAN vs local/website builds
if (on_cran) {
  knitr::opts_chunk$set(
    dpi = 72,           # Lower DPI for CRAN
    fig.width = 5,      # Smaller figures
    fig.height = 4,
    out.width = "70%"
  )
}
...
```{r check_suggests, include = FALSE}
# This vignette relies on optional (Suggests) packages. If any are missing the
# code chunks are not evaluated, so that the package can still be built and
# checked without them.
required_pkgs <- c("SuperLearner", "vimp", "quadprog", "glmnet", "nprobust", "ggplot2")
missing_pkgs <- required_pkgs[!vapply(required_pkgs, requireNamespace, logical(1), quietly = TRUE)]
has_deps <- length(missing_pkgs) == 0
knitr::opts_chunk$set(eval = has_deps)
```

```{r missing_note, eval = !has_deps, echo = FALSE, results = "asis"}
cat(
  "> **Note:** the code in this vignette was not evaluated because the following ",
  "optional packages are not installed: ", paste0("`", missing_pkgs, "`", collapse = ", "),
  ". Install them with `install.packages(c(", paste0('"', missing_pkgs, '"', collapse = ", "),
  "))` to run the examples.\n", sep = ""
)
```
...
results <- construct_pseudo_outcomes(prepped_data, y, a) %>%
  estimate_QoI(x1, x2, x3, x4, x5)
```
...
for (cov in c("x1", "x4", "x5")) {
  gp <- ggplot(filter(results, estimand == "MCATE", term == cov)) +
    geom_abline(intercept = 0, slope = 0, linetype = "dashed") +
    geom_ribbon(
      aes(
        x = value,
