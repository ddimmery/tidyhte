...
inference. `tidyhte` addresses these challenges by:

1.  Implementing state-of-the-art doubly-robust methods with automatic
    cross-validation
...
`tidyhte` is designed to support research across multiple domains where
understanding treatment effect variation is crucial:

- **Clinical Trials**: Identify patient subgroups that benefit most from
  medical treatments
- **Policy Evaluation**: Understand which populations are most affected
  by policy interventions
- **Technology & A/B Testing**: Optimize product features for different
  user segments
- **Economics**: Study heterogeneous effects of economic policies across
  demographics
- **Education**: Evaluate differential impacts of educational
  interventions
...
`vignette("methodological_details")`, which includes a quasi-real world
...
When choosing machine learning algorithms for the ensemble, consider a
progression like the following based on your subject-matter expertise:

- **Start simple**: `"SL.glm"` (linear models) for initial exploration
- **Add interactions**: `"SL.glm.interaction"` when you expect treatment
  effects to vary with covariates
- **Regularization**: `"SL.glmnet"` for higher-dimensional data
  (e.g. many interactions, note `SL.glmnet.interaction`) or when
  overfitting is a concern
- **Flexible models**: `"SL.ranger"` (random forests) or `"SL.xgboost"`
...
`SuperLearner::listWrappers()` for all available options. Adding any
`SL.*` learner requires the optional `SuperLearner` package; `tidyhte`
will offer to install it if it is missing.

# Installation

Install the released version of tidyhte from
...
The core estimation pipeline only depends on a small set of tidyverse
packages. Several features rely on optional packages which are only
loaded when used; `tidyhte` will prompt you to install them the first
time they are needed:

- `SuperLearner`: machine learning ensembles for the nuisance models
  (`SLEnsemble_cfg`, `add_*_model()`). Without it, `basic_config()`
  falls back (with a warning) to a single base-R `glm` per nuisance
  model (`GLM_cfg`), which is far less flexible.
- `vimp` and `quadprog`: non-linear variable importance (`add_vimp()`
  with the default `linear_only = FALSE`).
- `nprobust`: kernel-smoothed effect surfaces for continuous moderators
  (`add_moderator("KernelSmooth", ...)`).
- `quickblock`: stratified cross-validation splits (`make_splits()` with
  stratification covariates).
- `WeightedROC`: the `AUC` propensity-score diagnostic.
- `glmnet`: the `SL.glmnet` and `SL.glmnet.interaction` learners.

To install everything at once:

``` r
install.packages(c("SuperLearner", "vimp", "quadprog", "nprobust", "quickblock", "WeightedROC", "glmnet"))
```
...
SuperLearner ensembles for treatment, outcome and effect models with
linear models (`"SL.glm"`). If `SuperLearner` is not installed, it
instead warns and uses a single base-R `glm` for each of these models;
adding any `SL.*` learner upgrades that model back to an ensemble.
...
HTE estimation with cross-validation and ensemble machine learning can
be computationally intensive. Plan accordingly for larger datasets or
analyses with many outcomes.
