skip_if_missing_suggests("SuperLearner", "glmnet", "nprobust", "WeightedROC")
...
n_rows <- (
  1 + # SATE estimate
    ...
    2 * 7 + 1 + # one row per model in the ensemble for each PO + ps for SL coefficient
    3 * 100 + # 100 rows per continuous moderator for local regression for MCATE and for PCATE
    (4 + 3) # 2 rows per discrete moderator level for MCATE and for PCATE
)
