...
#' @examples
#' MCATE_cfg$new(cfgs = list(x1 = Stratified_cfg$new(covariate = "x1")))
...
MCATE_cfg <- R6::R6Class("MCATE_cfg",
  public = list(
    ...
    #' @examples
    #' cfg <- MCATE_cfg$new(cfgs = list(x1 = Stratified_cfg$new(covariate = "x1")))
    #' cfg <- cfg$add_moderator("x2", Stratified_cfg$new(covariate = "x2"))
...
#' @examples
#' PCATE_cfg$new(
#'    cfgs = list(x1 = Stratified_cfg$new(covariate = "x1")),
...
#'
#' Non-linear variable importance (the default) requires the optional `vimp` and
#' `SuperLearner` packages; `linear_only = TRUE` requires neither.
#' @examples
#' VIMP_cfg$new(linear_only = TRUE)
...
#' @examples
#' mcate_cfg <- MCATE_cfg$new(cfgs = list(x1 = Stratified_cfg$new(covariate = "x1")))
#' pcate_cfg <- PCATE_cfg$new(
#'    cfgs = list(x1 = Stratified_cfg$new(covariate = "x1")),
#'    model_covariates = c("x1", "x2", "x3"),
#'    num_mc_samples = list(x1 = 100)
#' )
#' vimp_cfg <- VIMP_cfg$new(linear_only = TRUE)
...
#'
#' When `SuperLearner` is not installed, the default model configurations fall
#' back to `GLM_cfg` (a single base-R GLM) with a warning. Explicitly pass
#' `Model_cfg` objects to control this.
...
HTE_cfg <- R6::R6Class("HTE_cfg",
  public = list(
    ...
    initialize = function(
      ...
      needs_default <- is.null(outcome) || is.null(treatment) || is.null(effect)
      if (needs_default && !package_present("SuperLearner")) warn_sl_fallback()
      if (is.null(outcome)) outcome <- default_model_cfg()
      if (is.null(treatment)) treatment <- default_model_cfg()
      if (is.null(effect)) effect <- default_model_cfg()
