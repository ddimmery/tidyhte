...
discrete_moderators <- rlang::exprs(x3)
...
Since in this example we know the exact propensity scores for all units, we use the `Known.cfg` config type.
...
for (cov in continuous_moderators) {
    qoi.list[[rlang::as_string(cov)]] <- KernelSmooth.cfg$new(neval = 100)
}
...
qoi.cfg <- QoI.cfg$new(
    ...
    # pcate = PCATE.cfg$new(cfgs = qoi.list, effect_cfg = regression.cfg, model_covariates = model_covariate_names),
    vimp = VIMP.cfg$new(model_cfg = regression.cfg),
    diag = Diagnostics.cfg$new(outcome = c("SL_risk", "SL_coefs", "MSE"))
)
```
...
data %>%
    ...
    estimate.QoI(!!!moderators, .HTE.cfg = cfg) -> results
```
