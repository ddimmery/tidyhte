...
The SuperLearner ensemble will automatically weight these models. Start with 2-3 algorithms and add complexity as needed. See `SuperLearner::listWrappers()` for all available options. Adding any `SL.*` learner requires the optional `SuperLearner` package; `tidyhte` will offer to install it if it is missing.
...
The core estimation pipeline only depends on a small set of tidyverse packages. Several features rely on optional packages which are only loaded when used; `tidyhte` will prompt you to install them the first time they are needed:

- `SuperLearner`: machine learning ensembles for the nuisance models (`SLEnsemble_cfg`, `add_*_model()`). Without it, `basic_config()` falls back (with a warning) to a single base-R `glm` per nuisance model (`GLM_cfg`), which is far less flexible.
- `vimp` and `quadprog`: non-linear variable importance (`add_vimp()` with the default `linear_only = FALSE`).
- `nprobust`: kernel-smoothed effect surfaces for continuous moderators (`add_moderator("KernelSmooth", ...)`).
- `quickblock`: stratified cross-validation splits (`make_splits()` with stratification covariates).
...
The `basic_config` includes a number of defaults: it starts off the SuperLearner ensembles for treatment, outcome and effect models with linear models (`"SL.glm"`). If `SuperLearner` is not installed, it instead warns and uses a single base-R `glm` for each of these models; adding any `SL.*` learner upgrades that model back to an ensemble.
